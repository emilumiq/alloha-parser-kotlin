...
class AllohaParser(context: Context) {
    ...
    interface Callback {
        ...
        /** CDN tokens (accepts-controls, authorizations) rotate per request; merge into OkHttp. */
            ...
            <script>
                ...
                iframe.onload = function() {
                    try {
                        ...
                        // ── Fetch intercept ───────────────────────────────────
                        var _fallbackHost = null; // 35d-a13-202gv...
                        var _primaryHost = null;  // ce1-048-404gv...
                        var _fallbackMasterUrl = null; // full fallback master.m3u8 URL
                        // Extract fallback host from bnsiData after it's set
                        ...
                        iframeWin.fetch = function(input, init) {
                            try {
                                ...
                                if (url && (url.indexOf('.m3u8') !== -1 || url.indexOf('.ts') !== -1)) {
                                    checkDone();
                                    extractFallbackHost();
                                    // If URL is on primary host, intercept and retry on fallback if 500
                        ...
                        // The player captures the original WebSocket constructor
                        // in __ws_factory BEFORE this code runs, so overriding
                        // iframeWin.WebSocket has no effect on connections created
                        // via __ws_factory. Instead we patch the PROTOTYPE --
                        // prototype method lookup is dynamic, so our patched
                        // send() method is called even on instances created with
                        ...
                        // Also override the constructor for any NEW WebSocket
                        // instances created after our code runs (backup).
