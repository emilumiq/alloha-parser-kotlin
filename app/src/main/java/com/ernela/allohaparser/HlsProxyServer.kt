...
class HlsProxyServer(
    ...
    // FIX #7: Instant URL rewriting for old-path segments after session refresh.
    // Old: https://cdn.../56/OLD_TOKEN.../seg-26-f1-v1.ts -> 403
    // New: https://cdn.../56/NEW_TOKEN.../seg-26-f1-v1.ts -> 200
    ...
    private suspend fun serveSegment(url: String, out: OutputStream) {
        ...
        val bytes = if (cached != null) {
            ...
            if (fetched != null) {
                ...
                // 403 on old URL -- try rewriting to current session path immediately
                ...
                if (rewrittenBytes != null) {
                    ...
                    // Rewrite also failed -- trigger async recovery
